...
const createPostgresPool = () => {
  ...
  return new Pool({ host, port, user, password, database, ssl: { rejectUnauthorized: false }, min: config.database.pool.min, max: config.database.pool.max, idleTimeoutMillis: 30000, connectionTimeoutMillis: 10000 });
};
...
export const poolReady = new Promise((resolve) => {
  async function connect() {
    ...
    if (process.env.VERCEL) {
      useSupabase = true; supabaseClient = createSupabaseClient(true);
      console.log('Using Supabase REST API (serverless fallback mode)'); resolve(); return;
    }
    if (config.supabase.url && config.supabase.dbPassword) {
      ...
      try {
        await p.query('SELECT 1');
        pool = p; useSupabase = false;
        console.log('Connected to Supabase PostgreSQL'); resolve(); return;
      } catch {
        await p.end().catch(() => {});
        console.log('Supabase direct connection unavailable, trying local PostgreSQL');
      }
    }
    pool = new Pool({ host: config.database.host, port: config.database.port, user: config.database.username, password: config.database.password, database: config.database.name, ssl: config.database.ssl ? { rejectUnauthorized: false } : false, min: config.database.pool.min, max: config.database.pool.max, idleTimeoutMillis: 30000, connectionTimeoutMillis: 5000 });
    useSupabase = false;
    console.log(`Connected to local PostgreSQL at ${config.database.host}:${config.database.port}`);
    resolve();
  }
  connect().catch(err => console.error('Failed to initialize database:', err.message));
});
poolReady.catch(() => {});
export { pool };
...
export const supabaseQuery = async (table, options = {}) => {
  ...
  for (const [key, value] of Object.entries(filters)) {
    ...
    if (key === 'or' || key === '_or') params.push(`or=${encodeURIComponent(String(value))}`);
    else if (typeof value === 'object' && value.op) params.push(`${key}=${value.op}.${encodeURIComponent(value.val)}`);
...
export const supabaseInsert = async (table, insertData) => {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, { method: 'POST', headers: supabaseHeaders, body: JSON.stringify(insertData) });
...
export const supabaseUpdate = async (table, data, filters) => {
  const paramStr = Object.entries(filters).filter(([, v]) => v !== undefined && v !== null).map(([k, v]) => `${k}=eq.${encodeURIComponent(String(v))}`).join('&');
  const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${paramStr}`, { method: 'PATCH', headers: supabaseHeaders, body: JSON.stringify(data) });
...
export const supabaseDelete = async (table, filters) => {
  const paramStr = Object.entries(filters).filter(([, v]) => v !== undefined && v !== null).map(([k, v]) => `${k}=eq.${encodeURIComponent(String(v))}`).join('&');
  const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${paramStr}`, { method: 'DELETE', headers: { ...supabaseHeaders, Prefer: 'return=minimal' } });
...
export const query = async (text, params) => {
  await poolReady;
  if (useSupabase) throw new Error('Raw SQL queries are not supported in Supabase REST fallback mode. Configure SUPABASE_DB_PASSWORD (and optionally SUPABASE_DB_HOST) for production SQL models.');
  const client = transactionStorage.getStore();
  const result = client ? await client.query(text, params) : await pool.query(text, params);
  if (config.env === 'development') console.log('Query executed', { text: text.substring(0, 100), duration: 0, rows: result.rowCount });
  return result;
};
...
export const getByTable = async (table, idColumn, idValue) => {
  ...
  const result = await query(`SELECT * FROM ${table} WHERE ${idColumn} = $1 LIMIT 1`, [idValue]);
...
export default { pool, query, getClient, getClientWithUser: getClient, transaction, transactionWithUser: transaction, closePool, setJwtContext, poolReady, useSupabase, supabaseQuery, supabaseInsert, supabaseUpdate, supabaseDelete, getByTable };
