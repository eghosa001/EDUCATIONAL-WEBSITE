...
export default function AdminDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-gray-50">
    <header className="bg-white border-b border-gray-200 h-14 flex items-center px-6 justify-between"><div className="flex items-center gap-3"><img src="/logos/the-guide-mark.svg" alt="THE GUIDE" className="w-8 h-8" /><span className="font-extrabold text-gray-900">THE GUIDE Admin</span></div><div className="flex items-center gap-4"><span className="text-sm text-gray-500">{user?.firstName} {user?.lastName}</span><button onClick={() => { logout(); window.location.href = '/login'; }} className="text-sm text-red-600 hover:text-red-700 font-medium">Logout</button></div></header>
    <div className="flex"><aside className="w-56 bg-white border-r border-gray-200 min-h-[calc(100vh-3.5rem)] p-3"><nav className="space-y-1">{items.map(([label,href]) => <a key={href} href={href} className="block px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-900">{label}</a>)}</nav></aside><main className="flex-1 p-6">{children}</main></div>
  </div>;
}
