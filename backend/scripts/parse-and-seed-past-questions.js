 ...
 *   SUPABASE_URL
 *   SUPABASE_SERVICE_ROLE_KEY (server-side key with required access)
 *   SUPABASE_DB_PASSWORD (for direct PostgreSQL access), or DATABASE_URL
 *
...
const SUPABASE_DB_HOST = process.env.SUPABASE_DB_HOST || (() => {
  try {
    return new URL(SUPABASE_URL).hostname.replace(/^/, 'db.');
  } catch {
    return null;
  }
})();
const DB_CONNECTION = process.env.DATABASE_URL || (
  DB_PASSWORD && SUPABASE_DB_HOST
    ? `postgresql://postgres:${encodeURIComponent(DB_PASSWORD)}@${SUPABASE_DB_HOST}:5432/postgres`
    : null
);

if (!SUPABASE_URL) throw new Error('Missing SUPABASE_URL.');
if (!DB_CONNECTION) throw new Error('Missing DATABASE_URL or SUPABASE_DB_PASSWORD.');
...
async function loadSubjectMap() {
  ...
  for (const row of result.rows) SUBJECT_MAP[row.name.toLowerCase()] = row.id;
...
async function main() {
  ...
  try {
    for (const file of files) {
      ...
      try {
        if (!file.public_url) throw new Error('Missing public_url');
