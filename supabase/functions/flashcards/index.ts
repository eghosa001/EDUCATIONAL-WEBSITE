...
const cors = (request: Request) => {
  ...
  return {
    "Access-Control-Allow-Origin": allowed ? origin : (allowedOrigins.length ? allowedOrigins[0] : "*"),
...
const validateCards = (value: unknown, count: number, topic: string) => {
  ...
  const forbidden = [
    "key topic in the nigerian curriculum",
    "read textbooks, attend classes, practice past questions",
    "builds foundational knowledge for waec",
  ...
  return value.map((item, index) => {
    ...
    const front = text(card.front, 1200);
    const back = text(card.back, 2500);
    const difficulty = text(card.difficulty, 20).toLowerCase();
    if (front.length < 8 || back.length < 15) throw new Error(`Flashcard ${index + 1} is too short`);
    const key = normalise(front);
    if (seen.has(key)) throw new Error(`Duplicate flashcard ${index + 1}`);
    seen.add(key);
    const combined = normalise(`${front} ${back}`);
    if (forbidden.some(phrase => combined.includes(phrase))) throw new Error(`Flashcard ${index + 1} contains generic filler`);
...
async function openAI(messages: Array<{ role: string; content: string }>, maxTokens: number) {
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "gpt-4o-mini", temperature: 0.25, max_tokens: maxTokens, messages }),
...
Deno.serve(async request => {
  ...
  try {
    ...
    const subjectId = text(body?.subjectId, 80);
    const topicId = text(body?.topicId, 80);
    const count = Number(body?.count || 20);
    if (!subjectId) return json(request, { error: "subjectId is required" }, 400);
    if (!Number.isInteger(count) || count < 5 || count > 30) return json(request, { error: "count must be an integer between 5 and 30" }, 400);

    const { data: subject, error: subjectError } = await admin.from("subjects").select("id,name").eq("id", subjectId).eq("is_active", true).maybeSingle();
    if (subjectError || !subject) return json(request, { error: "Selected subject was not found" }, 404);

    let topicQuery = admin.from("topics").select("id,name,description,learning_objectives").eq("subject_id", subjectId).eq("is_active", true);
    if (topicId) topicQuery = topicQuery.eq("id", topicId);
    const { data: topics, error: topicError } = await topicQuery.limit(topicId ? 1 : 20);
    if (topicError || !topics?.length) return json(request, { error: "Selected curriculum topic was not found" }, 404);
    ...
    const title = `${topic.name} — AI Flashcards`;
    const { data: saved, error: saveError } = await admin.from("flashcards").insert({
      subject_id: subject.id,
      topic_id: topic.id,
      title,
      description: `Curriculum-grounded flashcards for ${topic.name}.`,
      cards,
      mode: "ai",
      is_public: false,
      created_by: user.id,
    ...
    console.error("Flashcard generation failed:", message);
    return json(request, { error: message }, /required|invalid|expected|duplicate|generic|grounded|short|not found/i.test(message) ? 400 : 500);
  }
});
