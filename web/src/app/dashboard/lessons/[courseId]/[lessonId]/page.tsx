...
function optionsOf(v: any) {
  return Array.isArray(v)
    ? v.map((x: any, i: number) => ({ id: String(x?.id ?? x?.label ?? String.fromCharCode(65 + i)), text: String(x?.text ?? x?.value ?? x) }))
    : v && typeof v === 'object'
      ? Object.entries(v).map(([id, x]: any) => ({ id, text: String(x?.text ?? x?.value ?? x) }))
      : [];
}
function answerOf(v: any) {
  return typeof v === 'string' || typeof v === 'number' || typeof v === 'boolean'
    ? String(v)
    : v && typeof v === 'object'
      ? String(v.id ?? v.label ?? v.answer ?? v.value ?? '')
      : '';
}
...
function LessonContent({ content }: { content: string }) {
  ...
  return <article className="lesson-prose text-[16px] leading-8 text-slate-700 dark:text-slate-200"><ReactMarkdown components={{
    ...
    h2: ({ children }) => <h3 className="mb-4 mt-10 text-2xl font-extrabold text-[#151A3A] dark:text-white">{children}</h3>,
    h3: ({ children }) => <h4 className="mb-3 mt-8 text-xl font-extrabold text-[#151A3A] dark:text-white">{children}</h4>,
    ...
    blockquote: ({ children }) => <blockquote className="my-6 border-l-4 border-[#151A3A] bg-stone-50 px-5 py-3 italic dark:bg-[#151A3A]">{children}</blockquote>,
  }}>{safe}</ReactMarkdown></article>;
}
...
export default function LessonPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const s = getSupabase();
        let cq = s.from('courses').select('id,title,slug,short_description,subject_id,class_id,term_id,status').eq('status', 'published').limit(1);
        cq = isUuid(courseRef) ? cq.eq('id', courseRef) : cq.eq('slug', courseRef);
        const { data: c, error: ce } = await cq.maybeSingle();
        if (ce) throw ce;
        if (!c) throw new Error('Course not found');

        const { data: ls, error: le } = await s
          .from('lessons')
          .select('id,course_id,topic_id,slug,title,description,written_content,learning_objectives,key_points,estimated_minutes,video_url,video_duration_seconds,content_type,is_free,is_published,order_index')
          .eq('course_id', c.id)
        ...
        const list = ls || [];
        const found = list.find((l: any) => l.id === lessonRef || l.slug === lessonRef || encodeURIComponent(String(l.slug || '')) === rawLessonRef);
        if (!found) throw new Error('Lesson not found');

        // Critical: commit the core lesson before loading any optional features.
        // A question bank, resource or AI failure must never prevent a valid lesson from opening.
        if (!cancelled) {
          setCourse(c);
          setLessons(list);
          setLesson(found);
        ...
        if (found.topic_id && c.class_id && c.subject_id) {
          optionalTasks.push((async () => {
            const { data: qData, error: qError } = await s
              .from('questions')
              .select('id,question_text,options,difficulty,marks,question_image_url')
              .eq('topic_id', found.topic_id)
              .eq('class_id', c.class_id)
              .eq('subject_id', c.subject_id)
              .eq('is_active', true)
              .limit(20);
            if (qError) {
              if (!cancelled) setPracticeNotice('Lesson opened successfully. Stored practice questions are temporarily unavailable.');
              return;
            }
            if (!cancelled) setQuestions((qData || []).map((x: any) => ({ ...x, generated: false })));
          })());
        }
        ...
        if (!cancelled) setError(e?.message || 'Unable to load lesson');
      } finally {
        if (!cancelled) setLoading(false);
      }
    })();
    return () => { cancelled = true; };
  }, [courseRef, lessonRef, rawLessonRef]);
    ...
    (async () => {
      ...
      try {
        const response = await sendAiTutorMessage({
          message: `Teach this lesson as an accurate Nigerian curriculum-aligned teacher. Lesson title: ${lesson.title}. Course: ${course?.title || 'Unknown'}. Existing notes: ${lesson.written_content || lesson.description || 'none'}. Learning objectives: ${(lesson.learning_objectives || []).join('; ') || 'none'}. Give a clear explanation, definitions, step-by-step teaching, accurate worked examples where appropriate, common misconceptions and a short recap. Do not invent syllabus claims.`,
        ...
        if (!cancelled) setTeacherText('');
      } finally {
        if (!cancelled) setTeaching(false);
      }
    })();
    return () => { cancelled = true; };
  }, [lesson?.id, token, course?.title]);
  ...
  const selectAnswer = async (id: string) => {
    ...
    try {
      ...
      setScore(x => x + (payload.data.result.isCorrect ? 1 : 0));
      setAttempted(x => x + 1);
    } catch (e: any) {
      setSelected(null);
      setError(e?.message || 'Unable to check answer');
    } finally {
      setChecking(false);
    }
  };
  ...
  const markComplete = async () => {
    ...
    try {
      ...
      setError(e?.message || 'Unable to mark lesson complete');
    } finally {
      setCompleting(false);
    }
  };
  ...
  const goTo = (target?: any) => {
    if (target) router.push(`/dashboard/lessons/${encodeURIComponent(course?.slug || course?.id)}/${encodeURIComponent(target.id)}`);
  };
  ...
  if (error && !lesson) return <div className="mx-auto max-w-2xl py-16 text-center"><h1 className="text-xl font-bold text-[#151A3A]">Unable to open lesson</h1><p className="mt-2 text-slate-500">{error}</p><Link href="/dashboard/lessons" className="mt-6 inline-flex rounded-xl bg-[#151A3A] px-5 py-2.5 font-semibold text-white">Back to lessons</Link></div>;
