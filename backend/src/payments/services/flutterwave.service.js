...
export const flutterwaveService = {
  async initializePayment({ amount, currency, customerEmail, txRef, redirectUrl, metadata = {} }) {
    ...
    return { status: 'success', reference: data.data.tx_ref, link: data.data.link, transactionReference: data.data.tx_ref };
  },
  ...
  async verifyPayment(txRef) {
    ...
    return { status: data.data?.status?.toLowerCase(), reference: data.data?.tx_ref, amount: Number(data.data?.amount), currency: data.data?.currency, transactionReference: data.data?.id?.toString(), gatewayResponse: data.data };
  },
  ...
  async handleWebhook(req) {
    ...
    if (!secretHash || typeof signature !== 'string' || signature.length !== String(secretHash).length || signature !== String(secretHash)) throw new AppError('Invalid webhook signature', HTTP_STATUS.FORBIDDEN, ERROR_CODES.AUTHENTICATION_ERROR);
    return payload;
  },
};
