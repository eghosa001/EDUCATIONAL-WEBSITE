...
export default function GlobalError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    // Keep production diagnostics out of the UI; Next.js/server logging can retain the digest.
  }, []);
      ...
      <body>
        <main className="flex min-h-screen items-center justify-center bg-stone-50 px-4 dark:bg-[#151A3A]">
          <section className="w-full max-w-md rounded-2xl bg-white p-8 text-center shadow-lg dark:bg-slate-900">
            ...
            <p className="mb-6 text-sm text-slate-500 dark:text-slate-400">THE GUIDE could not load this page. Please try again.</p>
