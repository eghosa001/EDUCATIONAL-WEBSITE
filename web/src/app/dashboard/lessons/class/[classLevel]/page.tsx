...
function canonicalClass(value: string) {
  ...
  const compact = raw.replace(/class\s*[ab]$/i, '').replace(/[^a-z0-9]/g, '');
  const primary = compact.match(/^(?:primary|p)([1-6])(?:[ab])?$/); if (primary) return `primary-${primary[1]}`;
...
export default function ClassCurriculumPage() {
  ...
  return (
    <div className="space-y-7">
      ...
      <section className="overflow-hidden rounded-3xl bg-[#151A3A] p-7 text-white shadow-xl sm:p-9"><div className="flex items-end justify-between gap-5"><div><div className="text-4xl">{meta.icon}</div><p className="mt-4 text-xs font-bold uppercase tracking-[0.18em] text-slate-300">THE GUIDE CURRICULUM</p><h1 className="mt-2 text-4xl font-extrabold">{meta.title}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">{meta.subtitle}</p></div><GraduationCap className="hidden h-14 w-14 text-white/70 sm:block" /></div></section>
