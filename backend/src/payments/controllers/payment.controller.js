...
async function processSuccessfulPayment({ reference, gateway, gatewayReference, amount, currency }) {
  ...
  try {
    await notificationService.create({ userId: completed.user_id, type: NOTIFICATION_TYPES.PAYMENT_SUCCESS, title: 'Payment Successful', body: `Your payment of ₦${paymentAmount.toLocaleString()} was successful.`, actionUrl: '/dashboard/subscriptions/billing', channel: 'in_app' });
