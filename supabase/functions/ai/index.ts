...
const parseJson = (content: string): unknown => {
  try { return JSON.parse(content); } catch {
    ...
    if (arrayMatch) { try { return JSON.parse(arrayMatch[0]); } catch { return null; } }
...
const validateQuiz = (value: unknown, expectedCount: number) => {
  if (!Array.isArray(value) || value.length !== expectedCount) throw new Error(`AI returned an invalid quiz; expected ${expectedCount} questions`);
  return value.map((item, index) => {
    ...
    const questionType = cleanString(q.questionType, 30).toLowerCase() || 'multiple-choice';
    ...
    if (!questionText || !explanation || !correctAnswer || options.length < 2 || options.length > 6) throw new Error(`Invalid quiz question at index ${index}`);
    ...
    return { questionText, questionType, options, correctAnswer, explanation, difficulty };
  });
};

const validateFlashcards = (value: unknown, expectedCount: number) => {
  if (!Array.isArray(value) || value.length !== expectedCount) throw new Error(`AI returned an invalid flashcard set; expected ${expectedCount}`);
  return value.map((item, index) => {
...
const validateStudyPlan = (value: unknown) => {
  ...
  if (!Array.isArray(plan.dailySchedule) || plan.dailySchedule.length < 1 || plan.dailySchedule.length > 90) throw new Error('AI returned an invalid study plan schedule');
  const dailySchedule = plan.dailySchedule.map((day, index) => {
    ...
    if (!Number.isInteger(dayNumber) || dayNumber < 1 || !Number.isFinite(hours) || hours <= 0 || hours > 24 || topics.length === 0) throw new Error(`Invalid study-plan day ${index + 1}`);
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (action === 'quiz' && (!Number.isInteger(Number(body.questionCount || 10)) || Number(body.questionCount || 10) < 1 || Number(body.questionCount || 10) > 30)) return json(request, { error: 'questionCount must be an integer between 1 and 30' }, 400);
    if (action === 'flashcards' && (!Number.isInteger(Number(body.count || 10)) || Number(body.count || 10) < 1 || Number(body.count || 10) > 50)) return json(request, { error: 'count must be an integer between 1 and 50' }, 400);

    const { data: reservedResult, error: reserveError } = await adminClient.rpc('consume_ai_request', { p_user_id: user.id, p_daily_limit: DAILY_AI_LIMIT });
    ...
    if (action === 'tutor') {
      ...
      tokensUsed = Number(response.usage?.total_tokens || 0);
      result = { quiz: { id: crypto.randomUUID(), questions: validateQuiz(parseJson(response.choices?.[0]?.message?.content || ''), count), createdAt: new Date().toISOString() } };
    ...
    const { data: usage } = await adminClient.from('ai_usage').select('tokens_used,conversations_started').eq('user_id', user.id).eq('date', today).maybeSingle();
    await adminClient.from('ai_usage').update({
      tokens_used: Number(usage?.tokens_used || 0) + (Number.isFinite(tokensUsed) ? tokensUsed : 0),
      conversations_started: Number(usage?.conversations_started || 0) + (conversationStarted ? 1 : 0),
    ...
    if (reserved && userId) {
      try {
        const supabaseUrl = Deno.env.get('SUPABASE_URL');
        const serviceRoleKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY');
        if (supabaseUrl && serviceRoleKey) await createClient(supabaseUrl, serviceRoleKey).rpc('release_ai_request', { p_user_id: userId });
      } catch (releaseError) { console.error('Failed to release AI quota:', releaseError instanceof Error ? releaseError.message : 'unknown error'); }
    }
